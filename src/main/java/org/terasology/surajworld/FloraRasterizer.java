...
public class FloraRasterizer implements WorldRasterizer {
    private Block glowBell;

    @Override
    public void initialize() {
        glowBell = CoreRegistry.get(BlockManager.class).getBlock("Core:GlowbellBloom");
    }
    ...
    public void generateChunk(CoreChunk chunk, Region chunkRegion) {
        ...
        for(Vector3i block : FloraFacet.getWorldRegion())if(FloraFacet.getWorld(block))  {
            chunk.setBlock(ChunkMath.calcBlockPos(block.add(0,1,0)),glowBell);
        }
    }
}