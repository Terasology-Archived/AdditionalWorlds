...
@Produces(DeadBushFacet.class)
@Requires(@Facet(value = SurfaceHeightFacet.class))
public class DeadBushProvider implements FacetProvider {
    ...
    public void process(GeneratingRegion region) {
        ...
        DeadBushFacet facet = new DeadBushFacet(region.getRegion(), border);
        ...
        for (BaseVector2i position : surfaceHeightFacet.getWorldRegion().contents()) {
            ...
            if (facet.getWorldRegion().encompasses(position.getX(), surfaceHeight, position.getY())
                && noise.noise(position.getX(), position.getY()) > 0.992) {
                facet.setWorld(position.getX(), surfaceHeight, position.getY(), true);
            }
        }

        region.setRegionFacet(DeadBushFacet.class, facet);
    }
